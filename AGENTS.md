...
* The root `Dockerfile` builds both projects in a multi-stage process:
  ...
  3. A `generate-sbom` stage based on the pinned Syft image (default
     `anchore/syft:v1.13.0`, overridable via the `SYFT_IMAGE` build argument)
     produces a combined image SBOM. The Syft binary resides at `/syft`, so it
     is invoked in exec form:
