...
This endpoint returns a masked token in the `X-XSRF-TOKEN` header. Axios reads the cookie and sends the
masked value in the `X-XSRF-TOKEN` header automatically. `/api/auth/refresh` refreshes
